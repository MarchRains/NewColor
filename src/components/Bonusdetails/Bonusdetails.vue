<template>
    <div style="position:absolute;top:0;width:100%;background:#f5f5f5;height:100%;display:flex; flex-direction:column">
        <!-- 顶部 -->
        <div style="z-index:999;">
            <Header title="奖金详情"></Header>
        </div>
        <!-- 游戏目录tab -->
        <div class="game_wrap" style="position:absolute" :style="{'top':isLiuHai?'0.8rem':'0.5rem'}">
        ...
        <!-- 赔率列表 -->
        <div class="playGame" style="overflow: scroll;" :style="{'margin-top':isLiuHai?'0.8rem':'0.5rem'}">
            <!-- height:6rem; -->
            <div style="overflow: scroll;">
                <div v-for="(menusAll,index) in menus" :key="index" style="font-size:0.14rem;">
                    <p style="overflow:hidden;color:#fa3956;line-height:0.4rem;">
                    ...
                    <ul>
                        <li v-for="(rebateAll,index) in rebate[menusAll.id]" :key="index" >	
                            <p  style="overflow:hidden;line-height:0.4rem;background:white;border-bottom:1px solid #f5f5f5;">
