<template lang="html">
    <div class="yo-scroll1" :class="{'down':(state===0),'up':(state==1),refresh:(state===2),touch:touching}" @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)">
        <section class="inner" :style="{ transform: 'translate3d(0, ' + top + 'px, 0)' }">
            ...
            <footer class="load-more">
                <slot name="load-more">
                    <!-- <span v-show="downFlag === false && ifdownFlag" style="font-size:0.14rem">上拉加载更多数据</span> -->
    ...
    export default {
        ...
        methods: {
            ...
            touchEnd(e) {
                ...
                if(bottom+20 <= this.offset && this.state === 0) {
                    //console.log(dy)
                   if(dy>100){
...
.yo-scroll1.touch .inner {
    transition-duration: 0;
}

.yo-scroll1.down .down-tip {
    display: block;
}

.yo-scroll1.up .up-tip {
    display: block;
}

.yo-scroll1.refresh .refresh-tip {
    display: block;
}
</style>
