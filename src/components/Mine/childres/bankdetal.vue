<template>
  <div style="top:0; background: #f5f5f5;width:100%;height:100%;overflow:hiddden;">
    ...
    <div ref="bankl" style="overflow:scroll;position:relative;">
      <p style="font-size: 0.16rem;text-align: left;padding-left: 0.1rem;line-height: 0.4rem;color: #666;" :style="{'margin-top':isLiuHai?'0.8rem':'0.5rem'}" >收款账号信息</p>
        ...
        <div class="yinhangxinxi bgc" style="margin-top:0.2rem;">
           <p  @click="openPicker">
              <span>存入时间</span>
              <span>{{nowdata}}</span>
              <span style="height:0.3rem;border:none;"> </span>
            </p>
             <p>
              <span>存入金额</span>
              <!--<span ref="inputt">{{chamount}}</span>-->
              <input ref="inputt" pattern="\d" maxlength="6" type="tel" placeholder="请输入金额" style="float:left;width:30% !important;text-align:left; font-size:0.14rem;line-height:0.3rem;margin:0.1rem 0;border:none;outline:none;" v-model="chamount" @keyup="keyup">
              <span style="height:0.3rem;border:none;"  @click="qingchu"> </span>
            </p>
            <p>
              <span>存款人姓名</span>
             <input type="text" placeholder="请输入存款人姓名" style="float:left;width:49%!important;text-align:left; font-size:0.14rem;line-height:0.3rem;margin:0.075rem 0;border:none;outline:none;" v-model="name">
            </p>
        </div>
        ...
        <p style="position:absolute;width:100%;z-index:0">
           <span  class="buttonn" style="font-size:0.14rem;margin-right:0.2rem;" @click="goBack" >上一步</span>  <span class="buttonn" style="font-size:0.14rem;" @click="goto" >我已支付</span>
    ...
    return {
      ...
      title:"转账资料",
        ...
        },
        {
          flex: 1,
          values: [
          '00分','01分','02分','03分','04分','05分','06分','07分','08分','09分','10分','11分','12分','13分','14分','15分','16分','17分','18分','19分','20分','21分','22分','23分','24分',
      ...
      this.$router.go(-1)
  	 ...
  	 onValuesChange(picker, values) {
         ...
         var n=mountdays.length;
          for(var i=0;i<31;i++){
             mountdays[i].style.display="block"
          }

          for(var i=days;i<n;i++){
            mountdays[i].style.display="none"
          }
