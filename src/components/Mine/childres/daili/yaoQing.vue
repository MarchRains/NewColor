<template>
  <div class="tab "  style="position:absolute;top:1.01rem;width:100%;height:100%;z-index:1000;margin-top:0.1rem;" :style="{'background':nodata?'#fff':'#f5f5f5'}">
            ...
            <div class="nodata">
             ...
             <div ref="codes">
                 <ul class="header" style="top:0">
                    <li v-for="(item,index) in resultsecode" :key="index" style="font-size:0.12rem;">
                        ...
                        <span style="width:12%;">{{item.status==8?'禁用':"启用"}}</span>
                        <span style="width:34%;" :id="item.code">
                            <span class="spann" @click="change(item.rebate,item.type)">修改</span>
                            <span class="spann" @click="removee" style="background: rgba(204,204,204,1);">删除</span>
                            <span class="spann" style="background: rgba(50,187,131,1);" @click="shared">分享</span>
                        </span>
                    </li>
...
computed:{
    ...
    nodata(){
        if(document.getElementsByClassName("nodata")[0].style.display = "none"){
            this.nodata = false;
        }
        return this.nodata;
    }
 ...
 methods:{
    ...
    //   }, 200);
     ...
     oning(){
         ...
         //console.log(reslu);
         //console.log(this.$store.state.resultsecode);
         
         
         if(reslu.code==200){
              this.$store.state.resultsecode=reslu.data;
            this.$nextTick(()=>{
            if(this.resultsecode ==''){
            document.getElementsByClassName("nodata")[0].style.display = "block";
            this.$store.state.nodata = this.nodata = true;
     ...
     sureing(){
        if(this.localRebate){
             var datas=base.getApi().modifyCode(this.$store.state.userid,this.itemcode,this.type,this.status,this.localRebate);
            this.$store.state.resultsecode=datas.data;            
 ...
 updated() {
     if(this.$store.state.nodata == false){
         this.nodata = false;
     }
 },
  components: {
