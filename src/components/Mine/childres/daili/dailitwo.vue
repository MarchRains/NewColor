<template>
  <div  style="overflow: hidden;position:fixed;top:0;width:100%;height:100%;background: #f5f5f5;z-index:1000;">
        <Header title="下级开户"></Header>
         <p class="maintop" style="font-size:0.15rem;background-color:#fff;" :style="{'margin-top':isLiuHai?'0.8rem':'0.5rem'}">
      ...
      <div class="main" ref="meno">
          ...
          <ul> 
              <li> 
               ...
               <li> 
                   <span>彩票返点</span><input type="button" :value="localRebate"  style="color:#666;padding-left:0;background: transparent;text-align: left;font-size:0.16rem;" class="jin vall">
                   <span class="jin" >
                       <span></span >
                        </span></li>
          </ul>
          <p  v-if="ifscrest" style="font-size:0.18rem;color:#666;line-height:0.5rem;color:red">初始密码为123456</p> 
          <p ref="pp" style="font-size:0.16rem;"></p>
          
          <div class="robate" :style="{'top':isLiuHai?'3.5rem':'3.1rem'}"> 
              <ul>
      ...
      <!-- <yaoqingma id="yaoqingma" :resultsecode="resultsecode"></yaoqingma> -->
       <yaoqingma id="yaoqingma" :style="{'top':isLiuHai?'1.25rem':'1.01rem'}"></yaoqingma>
  </div>
</template>
      ...
      getApi(){
        ...
        base.getApi().codeList1(this.pageNumber,this.$store.state.pageSize,userid).then((res)=>{
                     this.resultsecode=res.data;
                      ...
                      console.log(this.$store.state.resultsecode.length)
                      if(this.$store.state.resultsecode.length!=0){
                          document.getElementsByClassName("nodata")[0].style.display = "none";
                      }
         });
