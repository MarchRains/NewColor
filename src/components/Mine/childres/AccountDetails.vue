<template>
    <div style="overflow: hidden;position:fixed;top:0;width:100%;height:100%;background: white;z-index:1000;">
        ...
        <headertop title="账户明细" :leftType="2" />
        <!-- 消息tab分类 -->
        <div class="nav-wrap">
            <div class="nav">
                <ul class="navlist" style="width: 10.23rem;">
                    <li @click="one('')" id="" class="isclick" style="width: .93rem;">全部类型</li>
                    <li v-for="(inte,index) in arrAll" :key="index"  @click="one(inte.split('-')[0])" style="width: .93rem;">{{inte.split('-')[1]}}</li>
                </ul>
            </div>
        </div>
        <!-- 明细列表 -->
   
      <div class="datalist-wrap"  ref="listwrapVue" :style="{top: wrapTop}">
            <!-- 无数据样式 -->
                  ...
                  @click="accoundetil">
                    <div class="detailtext">
                        ...
                        <div class="textbottom">
                            ...
                            <div class="timer" >{{account.addTime | dateFmt('YYYY-MM-DD HH:mm:ss')}}</div>
                        </div>
                    </div>
                </li>
            </ul>
    ...
    winner() {
      ...
      for (var i = 0; i < this.accountAll.length; i++) {
        if (this.accountAll[i].type == this.type[0]) {
          ...
          this.$refs.winning[i].style.color = "#000";
        }else if (this.accountAll[i].type == this.type[8]) {
          this.$refs.winning[i].innerHTML = this.typename[8];
          this.$refs.winning[i].style.color = "#000";
        }
      }
    },
