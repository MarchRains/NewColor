<template>
  <div ref="chargedetal" class="chargedetal" style="position:absolute;left:0;top:0;width:100%;;background: white;z-index:99999;overflow:hidden;">
     
        
         <headertop :title="title" :datal="datal" :goDetal="mycollect"> </headertop>
        
 <div class="zhushou"  ref="zhuzhein" v-if="zhuzhein" :style="{'margin-top':isLiuHai?'0.8rem':'0.5rem'}">
        ...
        <div style="height:100%;">
            <div class="inputt" :style="{'margin-top':isLiuHai?'0.9rem':'0.6rem'}">
                <span >请输入充值金额</span>
                <input pattern="\d" type="number" maxlength="10"  id="inputt" v-sliceString="10" ref="inputtt" :value="val" @keyup="keyup" style="color:#D71437;">
                <span>元</span>
            </div>
            <div class="jine">
                <ul id="bgcc">
            ...
            <div class="chongzhi" >
                <p class="pp" style="line-height:0.36rem;color:gray;font-size:0.12rem;text-align:left;padding-left:0.2rem;background:#f5f5f5">请选择充值方式 ( 如有问题，请联系
                ...
                <div >
                    <ul class="zhifuu" ref="zhifuu">
                        <li v-for="(item,index) in list" :key="index" @click="chongzhiin" :name="item.id" >
                        	<div class="paidinn">
                                <img class="kijg" style="width: 0.28rem !important;height: 0.21rem !important;position: absolute;left: 0;" src="../../../assets/images/isHot.png" alt="" />
 ...
 methods:{
     serviceFuncA(){
      ...
      disnone(){
		...
		getulheight(){
		 		...
		 		// var ulheight=bodyheight-45-inputtheight-jineheight-ppheight;
		 		this.$refs.zhifuu.style.height=document.getElementsByTagName("body")[0].offsetHeight-45-40-130-36+"px"
     ...
     getapi(){
        var flash=localStorage.getItem("flash");
            //console.log(flash)
            if(flash==1){
                   this.$refs.chargedetal.style.height=document.getElementsByTagName("html")[0].clientHeight+"px";
     ...
     keyup(){
       if (this.$refs.inputtt.value.length>10) {
        this.$refs.inputtt.value="";
        this.val=this.$refs.inputtt.value;
        $.tips('充值金额最大不能超过99亿元')
      }else{
        let ex = /(^[0-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/;
        if (!ex.test(this.$refs.inputtt.value)) {
          $.tips("您输入的数字有误请重新输入");
        this.$refs.inputtt.value="";
        this.val=this.$refs.inputtt.value;
        }
      }
      //console.log("refs", this.$refs.inputtt.value);
      ...
      for (var i = 0; i < lis.length; i++) {
        if (this.$refs.inputtt.value !== parseInt(lis[i].innerHTML)) {
    ...
    chongzhiin(){
          ...
          for(var i=0;i<this.$store.state.rechargedetal.length;i++ ){
              ...
              if(idname==this.$store.state.rechargedetal[i].id){
                 ...
                 if(!!openDecimal){
                     amount=Number(this.val)+ Number(Math.random().toFixed(openDecimal));
                 }else{
                ...
                //  console.log(this.$store.state.rechargedetal)
                  ...
                  this.$store.state.plateformConfigId=this.$store.state.rechargedetal[i].plateformConfigId;
                this.$store.state.headername=this.$store.state.rechargedetal[i].name;
              }
          }
        ...
        if(amount>1){      
                ...
                base.getApi().newRechargenew(userId, amount, plateformConfigId).then((res)=>{
                    this.result=res
                     console.log(this.result)
                        ...
                        if(turnType == 2) {
                            ...
                            if(this.$store.state.isid){
                                var jj = JumpOpen(qrCode, '_bank' + Math.random());
                            }else{
    ...
    getapitwo(){
        ...
        if(amount){
                if(turnType==1||turnType==2){
                this.result=base.getApi().newRecharge(userId, amount, plateformConfigId);
                //console.log( this.result);
                if( this.result &&  this.result.code == 200){
