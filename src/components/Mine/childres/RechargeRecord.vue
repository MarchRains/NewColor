<template>
    <div style="overflow: hidden;position:fixed;top:0;width:100%;height:100%;background: white;z-index:1000;">
        <!-- 顶部 -->
         <headertop :title="title" :leftType="2"> </headertop>
        <!-- 消息tab分类 -->
        <div class="nav-wrap">
            <div class="nav">
                <ul class="navlist" style="width:100%;">
                    <li @click="one('')" class="isclick" style="width:20%;font-size:0.13rem;">全部</li>
                    <li v-for="(inte,index) in dictListAll" :key="index"  @click="one(inte.split('-')[0])" style="width:20%;font-size:0.13rem;">{{inte.split('-')[1]}}</li>
                </ul>
            </div>
        </div>
        <!-- 明细列表 -->
        <div class="datalist-wrap" ref="listwrapVue" :style="{top: wrapTop}" style="overflow: scroll;position: absolute;bottom: -.45rem;left: 0;right: 0;">
            <!-- 无数据样式 -->
                    ...
                    <ul class="detaillist" >
                        <li v-for="(bettings,index) in rechargeAll" :key="index" :name="bettings.id" :id="bettings.status" @click="click">
                                <div class="detailtext" >			
                                    <div class="texttop">				
                                        <h4 ref="winning"></h4>				
                                        <span class="bgspan">+{{bettings.finalAmount|mory(bettings.amount)}}元</span>			
                                    </div>			
...
import headertop from '../../header/header'
  export default{
        ...
        computed:{
            wrapTop() {
                return (this.$store.state.headerHeight + 45) / 100 + 'rem';
            }
        },
        methods: {
         onRefresh(done) {
            ...
            styleCss(){
                ...
                //document.getElementsByClassName("datalist-wrap")[0].style.height = '92%' ;
                // document.getElementsByTagName("body")[0].clientHeight-50+'px'
            },
            winner(){
