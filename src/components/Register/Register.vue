<template>
  <div style="height:100%;background:#fff;overflow:hidden">
    <p class="pde" style="width: 100%;background:linear-gradient(90deg, #fa3956, #ff6b52);position: fixed;z-index:99999;" v-if="isApp"></p>
    <div class="ghd">
      <headertop title="注册" :leftType="2" />
    </div>
      ...
      <div class="ocontent">
        <ul class="list">
          ...
          <li style="position: relative;">
           ...
           <li class="telphone" v-if="iftelphone">
            <span>手机号</span>
            <input  name=""  :placeholder="ifyzphone?'请输入电话号码(必填)':'请输入电话号码(选填)'" ref="input6" v-model="result6" />
          </li>
          <li class="email" v-if="ifemail">
            <span>邮箱</span>
            <input  name=""  :placeholder="ifyzemail?'请输入邮箱(必填)':'请输入邮箱(选填)'" ref="input7" v-model="result7" />
          </li>
        ...
        <div class="notice">
          <div class="pclick clear">
            <p style="width:0.2rem;height:0.2rem;margin-left:0;border:1px solid #d9d9d9;" @click="removeisclick"> 
              <img src="../../assets/source/imgs/images/yes_05.png" alt="" :class="{ bgroung: ifchoose }" style="width: 0.2rem;">
            </p>
  ...
  export default {
    ...
    // 是否显示邮箱
     var ifemail=JSON.parse(localStorage.getItem("sysConfigMap")).emailShowAndValidate.cvalue;
      ...
      this.auth()
      //console.log(JSON.parse(localStorage.getItem("sysConfigMap")).emailShowAndValidate.evalue);
