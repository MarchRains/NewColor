<template>
  <!-- font:16px '黑体'!important; -->
  <div style="overflow: hidden; position:fixed;top:0;height:100%;background:#fff;z-index:1000;width:100%;">
  <div class="maxonly" style="position: absolute;top: 0;left: 0;right: 0;bottom: 0;height: auto;">
    <!-- 顶部栏 -->
    <headertop title="购彩大厅" :rightType="2">
      <template slot="rightContent">
        <router-link :to="{path:'/Buycolornew'}">
            <a class="header-oper">
    ...
    <div class="wrap-trend">
			<div class="trend">
				<div class="trend-ul">
          <ul id="trend">
            <li @click="showAll()" class="dataall" datatype="all">全部</li>
    ...
    <!-- 彩种 -->
    <div class="wrapper" id="wrapper" :style="{top: wrapperTop}">
			<div class="content content-trend content-tabbar">
				<div style="display: none;" class="lottery">
          <div class="divAll" v-for="(item,index) in datalist">
            ...
            <a :index="index" :id="item.id" :name="item.showType" :lname="item.name" :status="item.status" v-if="item.status!=8" class="weui-media-box weui-media-box_appmsg bgf">
              ...
              <div class="weui-media-box__bd">
                ...
                <div style="font: 15px 黑体 !important;height: .2rem;" class="weui-media-box__title lo-num" :id="'lotteryResultDiv_'+item.id" v-else-if="item.showType == '4' && item.lotteryDraw.closeTime > 0 && !!item.lotteryDraw.result">
...
export default {
  ...
  methods: {
	...
	showAll() {
		...
		window._buycolor_selected_datatype = 'all';
		$(".dataall")
			.addClass("on")
			.siblings("li")
			.removeClass("on");
	...
	tap() {
		...
		$("#trend").on("click", ".litype", function() {
			...
			// 购彩大厅 菜单当前选择项datatype值
			window._buycolor_selected_datatype = $(this).attr("datatype");
			$this.datalist = k;
			k = [];
			// var $this = this;
			setTimeout(function() {
          ...
          if (!!this.$store.state.iscollect[yhuserId.id + ""]) {
            ...
            for (var i = 0; i < $(".lottery .scAll").length; i++) {
              for (var t = 0; t < sccz.data.length; t++) {
                if (
                  $($(".lottery .scAll")[i]).attr("name") == sccz.data[t].lottery.code
                ) {
                  $($(".lottery .scAll")[i])
                    .find("img")
                    .attr("src", this.collect[1]);
                  $($(".lottery .scAll")[i])
                    .find(".toward")
                    .html("&nbsp;已收藏");
                }
            ...
            base
              ...
              .then(res => {
                ...
                for (var i = 0; i < $(".lottery .scAll").length; i++) {
                  for (var t = 0; t < sccz.data.length; t++) {
                    if (
                      $($(".lottery .scAll")[i]).attr("name") ==
                      sccz.data[t].lottery.code
                    ) {
                      $($(".lottery .scAll")[i])
                        .find("img")
                        .attr("src", this.collect[1]);
    ...
    reoter() {
      ...
      $(".lottery").on("click", "a", function() {
        ...
        if ($(this).attr("name") == 0) {
          ...
          // },300);
          $this.$store.state.urlgoucai = $(this)
            .find("img")
            .attr("src");
        ...
        if (yhuserId != null) {
          ...
          if (yhuserId.type != 2) {
            if (
              ...
              base
                .getApi()
                .userCollect1(yhuserId.id, $(this).attr("index"))
                .then(res => {
                  $(this)
                    .children("img")
                    .attr("src", $this.collect[1]);
              ...
              base
                .getApi()
                .collectCancel1(yhuserId.id, $(this).attr("index"))
                .then(res => {
                  $(this)
                    .children("img")
                    .attr("src", $this.collect[0]);
	...
	mounted: function() {
		...
		if (version) {
			...
			this.colornew();
			this.reoter();
			setTimeout(function() { $this.tap(); }, 10);
