<template>
    <div style="overflow: hidden;position:absolute;top:0;width:100%;height:100%;background: white;z-index:1000;">
		...
		<div id="layout"  style="background-color: #f5f5f5;">
			<div class="header_wrap layoutid" :style="{'height':isLiuHai?'0.8rem':'0.5rem'}">
				<div class="header" :style="{'top':isLiuHai?'0.45rem':'0.05rem'}">
					<div @click="gobacka" class="set" :style="{'top':isLiuHai?'0.2rem':'0.25rem'}">
						<i class="iconfont" >&#xe61f;</i>
					</div>
					<h2 >投注列表</h2>
				</div>
			...
			<div class="ocontent" >
				<div class="timecount">
					距<span id="lotteryNoSpan">{{thisdatalist.lotteryDraw.shortNextNo}}</span>期截止<p><span id="remainTimeSpan"><p class="spans">{{thisdatalist| mytimeAll('shi1')}}</p><p class="spans">{{thisdatalist| mytimeAll('shi2')}}</p>:<p class="spans">{{thisdatalist| mytimeAll('fen1')}}</p><p class="spans">{{thisdatalist| mytimeAll('fen2')}}</p>:<p class="spans">{{thisdatalist| mytimeAll('miao1')}}</p><p class="spans">{{thisdatalist| mytimeAll('miao2')}}</p></span></p>
				</div>
				<div class="obet">
					<div class="selectbet">
						<div class="selecttop">
						...
						<div class="numtext" style="display: block;!important;">
							统一金额
                            <input style="float: right;height: .3rem;width: 40%;
                            line-height: .3rem;border: 0.01rem solid #ccc;border-radius: 0.04rem;background: #fff;margin: 0.05rem 0 0 .1rem;
                            font-size: .14rem;text-indent: 0.05rem;" pattern="[0-9]*" :dataId="index" maxlength="6" class="gl-txt" id="settingAllAmount" type="tel" placeholder="" value=""  onkeyup="value=value.replace(/[\W]/g,'')">
						</div>
					</div>
					<!--注单列表-->
                    <ul class="hkbetlist" style="height: 301px; overflow-y: auto;">
                        <li v-for="(picktherendermin,index) in picktherender" :key="index">
                            ...
                            <div class="liright">
                                ...
                                <div class="licoenten">
                                    赔率：<span>{{picktherendermin.peilv|stringFilter}}&nbsp;&nbsp;&nbsp;</span>返利：
                                    <span>{{picktherendermin.fanlv}}%</span>
                                </div>
                                <div class="detail">
                                    <span class="zus">{{picktherendermin.zushu}}</span>注 <span class="moayAll">{{picktherendermin.zonge|tofixed}}</span>元 
                                     <input pattern="[0-9]*" :dataId="index" maxlength="6" id="" class="gl-txt" type="tel" :placeholder="picktherendermin.zonge |tofixed" v-model="picktherendermin.isdanzuval">
                                    <!-- <input pattern="[0-9]*" :dataId="index" maxlength="6" id="" class="gl-txt" type="tel"
                                     :placeholder="picktherendermin.isdanzu"
                                      :value="Number(picktherendermin.isdanzuval).toFixed(2)"> -->
                                </div>
                                <div class="">
				...
				<!--追加期数-->
				<div class="wrap-addto" :style="{'bottom':isid?'0.5rem':'0.5rem'}"> 
					<div class="addto">
             ...
             <!-- :style="{'bottom':isLiuHai?'0.52rem':'0.52rem'}" -->
			<div class="wrap-foot" :style="{'bottom':isLiuHai?'0':'0'}">
				<div class="foot">
					<div class="btn" id="qk">
						清空
					</div>
    ...
    export default{
        ...
        methods:{
            ...
            moerys(){
                ...
                this.moeryAll = this.moeryAll * this.followup
            },
            defalts(){
            ...
            jsAll(){
                ...
                $('.hkbetlist').css({'height':document.getElementsByTagName("body")[0].offsetHeight
                -document.getElementsByClassName("layoutid")[0].offsetHeight
                -document.getElementsByClassName("selectbet")[0].offsetHeight
                -50-50-50}) 
                var $this = this;
                ...
                //          $this.picktherender[+$(this).attr('dataId')].isdanzuval = '2';
                ...
                // })
                  ...
                  //修改单注金额
					...
					if (pool.indexOf(keyCode) < 0) return false;
					if ([110, 190].indexOf(keyCode) >= 0 && /\./.test(val)) return false;
					if (/^\d*\.\d{2}$/.test(val) && keyCode !== 8) return false;
				};
				// 修改单注金额
				$('.hkbetlist').on('focus', '.gl-txt', function() {
                    var el = $(this);
                    el.data('oldValue', +el.val());
					$this.picktherender[+el.attr('dataId')].isdanzuval = '';
				}).on('blur', '.gl-txt', function() {
                    var el = $(this);
					var target = $this.picktherender[+el.attr('dataId')];
					if(!el.val()) {
						target.isdanzuval = el.data('oldValue');
						target.zonge = target.zushu * target.isdanzuval;
					}
					target.isdanzuval = parseFloat(target.isdanzuval).toFixed(2)
					$this.moerys();
				}).on('keydown', '.gl-txt', _keydown).on('keyup', '.gl-txt', function() {
					var val = parseFloat($(this).val() || 0);
					var target = $this.picktherender[+$(this).attr('dataId')];
					if (!val) {
						target.isdanzuval = '';
						return;
					}
					target.isdanzuval = $(this).val();
					target.zonge = target.zushu * target.isdanzuval;
					$this.moerys();
				});

                //统一金额
				var _settingAllAmount = $('#settingAllAmount');
				_settingAllAmount.on('keydown', _keydown).on('keyup', function() {
					var val = parseFloat($(this).val() || 0);
					for (var i = 0; i < $this.picktherender.length; i++) {
						$this.picktherender[i].isdanzuval = val.toFixed(2);
					}
					for(var j = 0; j < $this.picktherender.length; j++) {
						var target = $this.picktherender[j];
						target.zonge = target.isdanzuval * target.zushu;
					}
					if(!val) {
						for(var i = 0; i < $this.picktherender.length; i++) {
							var target = $this.picktherender[i];
							target.isdanzuval = $this.er.toFixed(2);
							target.zonge = target.isdanzuval * target.zushu;
						}
					}
					$this.moerys();
            ...
            buyConfirm(){
                ...
                if(resultJson.code == 200){
                    ...
                    changeLocal();
                             window.localStorage.setItem("nowShortNo",lotteryJson.lotteryDraw.nextNo)
                    this.$router.push({path:'/touzhuSuccess'});
                    
                }else{
                    if(resultJson.code == 2008){
                        //弹出confirm框,提示用户已经关盘,是否购买下期
                        $.tips("参数异常");
                        
                    }else if(resultJson.code == 2009){
