<template>
   <div class="maxisonly" style="font-size:.14rem;overflow: hidden;position:fixed;top:0;width:100%;height:100%;background: white;z-index:1000;">
       <div class="modol" style="" v-if="!iswanfa"></div>
		<header :style="{'height':isLiuHai?'0.8rem':'0.5rem'}">
		   <span class="" @click="goback" :style="{'margin-top':isLiuHai?'0.5rem':'0rem'}"><i class="iconfont" >&#xe61f;</i></span>
           <div id="tombola_Xdll" :class="{'bgcs':iswanfa}" :style="{'margin-top':isLiuHai?'0.5rem':'0.1rem'}">
			   <span v-if="kindAll.id==namea"  v-for="(kindAll,index) in kind.data" class="tombola_Xd" :key="index" >{{kindAll.name}}</span>
			   <i style="display: inline-block;height: 30px;width:30px;text-align:center;border-radius: 0px 0px 5px 5px;font-size: 0.16rem;line-height: 30px;transform: rotate(-90deg);" class="iconfont" v-if="!iswanfa">&#xe61f;</i></div>
		</header>
		<div class="tombola" v-if="!iswanfa" :style="{'top':isLiuHai?'0.9rem':'0.6rem'}">
			<ul class="lis">
		...
		<div class="max" style="    overflow: hidden;">
			<div class="tap"  v-if="!iswanfa">
				<span index="1" class="span1 on" >彩种介绍</span>
				<span index="1" class="span2">玩法规则</span>
			</div>
			<div v-if="controller=='haver'" class="main" style="padding:0.1rem;background:#fff;font-size:0.16rem">
			...
			<div v-else v-html="letter" class="main" style="padding:0.1rem;background:#fff;font-size:0.16rem">
				{{letter}}
			</div>
		</div>
   </div>
...
export default {
  ...
  mounted: function() {
    ...
    this.effect();
    this.isplay();
    $('.MsoNormal span').css({fontSize:'0.16rem'})
    $('.p span').css({fontSize:'0.16rem'})
  },
