<template>
    <div style="overflow: hidden;position:fixed;top:0;width:100%;height:100%;background: white;z-index:1000;" id = "layout">
        <!-- <Header :title="lotteryDrawList[0].lottery.name" ></Header> -->

         <!-- <div class="index_header" style="position:fixed;top:0;height:0.45rem;">
		          <div @click="backtrack" style="top: 33px; position: absolute;transform: translateY(-50%);color: #fff;width:20%;height:0.45rem;">
                    <i class="iconfont icon-mjiantou-copy" style="    font-size: .24rem;position: absolute;line-height: 45px;margin-left: 12px;left:0;height:0.45rem;"></i>
             </div>
		    	<h3 >{{lotteryDrawList[0].lottery.name}}</h3>
                <div v-show="isShowBack" class="trendtu" style="height:0.45rem;top: 33px;position: absolute;transform: translateY(-50%);color: #fff;width:20%;right:0">走势图</div>
	    	</div> -->

        <div class="header_wrap" style=" position: fixed;height: .85rem;" :style="{'padding-top':isLiuHai?'0.3rem':'0.2rem'}">
        <div class="header">
            <div @click="backtrack" class="set ">
            <i class="iconfont icon-mjiantou-copy" style="position: absolute;"></i>
            </div>
            <div class="setrigth" style="position: relative;width: 87%;height: .65rem;background-color: white;border-radius: 0.5rem 0 0 .5rem;position: absolute;right: 0;top: .1rem;">
                <img style="position: absolute;width: 0.55rem;left: 0.1rem;top: 0.054rem;rem;" alt="" :src="urlclick">
                <div style="    position: absolute;left: .8rem;top: .1rem;box-sizing: border-box;font-size: .13rem;text-align: left;">
                ...
                <div style="position: absolute;right: .2rem;top: .1rem;text-align: right;font-size: .16rem;">
                    <section v-if="name1=='ff11x5'||name1=='jsssq'||name1=='fflhc'||name1=='js3d'" style="    margin-bottom: -.065rem;color: red;font-size: .24rem;">
                    ...
                    <section v-else style="color: red;font-size: .24rem;    margin-bottom: -.065rem;">
        ...
        <div class="content" id="contented" style="position: relative;height:100%;overflow:scroll;">
             ...
             <!-- <mt-loadmore  :top-method="loadTop"  style="background:white;"  ref="loadmore"  :max-distance="150" @top-status-change="handleTopChange" >
				<div class="list weui-pull-to-refresh" style="height: 800px;font-size:.13rem">
					...
					<ul  v-if="lotteryDrawListFirst=='11'" class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">
                        <!-- 幸运扑克 -->
                    ...
                    <!-- 双色球 -->
					<ul v-else-if="name1=='ssq'||name1=='jsssq'" class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">  
                    ...
                    <ul  v-else-if="lotteryDrawListFirst=='2'" class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">
                        <!-- 重庆时时彩 -->
                       <li v-if="!!itemAll.result" v-for="itemAll in lotteryDrawList" >	<div class="liwrap"><div class="litop"><span> 第{{itemAll.lotteryNo}}期</span>
                    ...
                    <ul  v-else-if="lotteryDrawListFirst=='6'" class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">
                        <!-- 极速快三 -->
                        <li v-if="!!itemAll.result" v-for="itemAll in lotteryDrawList"><div class="liwrap"><div class="litop">
                            <span> 第{{itemAll.lotteryNo}}期</span><span>{{itemAll.addTime | dateFmt('YYYY-MM-DD HH:mm:ss')}}</span></div>
                            <div class="libottom">
                    ...
                    <ul v-else-if="name1=='jspks'||name1=='xyft'||name1=='xysm'||name1=='bjpks'" class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">
                        <!-- 北京pk十 -->
                        <li v-if="!!itemAll.result" v-for="itemAll in lotteryDrawList"><div class="liwrap"><div class="litop">
                            <span> 第{{itemAll.lotteryNo}}期</span><span>{{itemAll.addTime | dateFmt('YYYY-MM-DD HH:mm:ss')}}</span></div>
                            <div class="libottom">
                            <div class="num_1">
                            <div v-for="item1 in itemAll.result.split(',')"  :style="'border-radius:1px;background:none;text-shadow: 0 1px #000, 1px 0 #000, -1px 0 #000, 0 -1px #000;background: linear-gradient(90deg, '+colorAll[item1]+', '+colorAll[item1+'1']+')'" class="hong">{{Number(item1)}}</div>
                            </div></div>
                            </div>
                        </li>
                    </ul>   
                    <ul  v-else-if="lotteryDrawListFirst=='4'" class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">
                        <!-- 北京28 -->
                        <li v-if="!!itemAll.result" v-for="itemAll in lotteryDrawList"><div class="liwrap"><div class="litop"><span>第{{itemAll.lotteryNo}}期</span>
                            <span>{{itemAll.addTime | dateFmt('YYYY-MM-DD HH:mm:ss')}}</span></div><div class="libottom">
                            <div class="num_1">
                    ...
                    <ul  v-else-if="lotteryDrawListFirst=='3'||lotteryDrawListFirst == '5'" class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">  
                        <!-- 重庆快乐十分 -->
                        <li v-if="!!itemAll.result" v-for="itemAll in lotteryDrawList"><div class="liwrap"><div class="litop"><span> 第{{itemAll.lotteryNo}}期</span>
                            <span>{{itemAll.addTime | dateFmt('YYYY-MM-DD HH:mm:ss')}}</span></div>		<div class="libottom"><div class="num_1">
                    ...
                    <ul  v-else class="lotterlist" :style="{'padding-top':isLiuHai?'1.1rem':'1.0rem'}">
                        <li v-if="!!itemAll.result" v-for="itemAll in lotteryDrawList"><div class="liwrap"><div class="litop"><span> 第{{itemAll.lotteryNo}}期</span>
                        <span>{{itemAll.addTime | dateFmt('YYYY-MM-DD HH:mm:ss')}}</span></div><div class="libottom">
                                ...
                                <div  v-if="index == '5'">
                                    <div  v-if="red.indexOf(itemminall) != '-1'"><div style="height: 27px;width: 27px;line-height: 27px;" class="hong">{{itemminall}}</div><span id="jia"> + </span></div>
                                    <div  v-else-if="green.indexOf(itemminall) != '-1'"><div style="height: 27px;width: 27px;line-height: 27px;" class="lv">{{itemminall}}</div><span id="jia"> + </span></div>
                                    <div  v-else="bulr.indexOf(itemminall) != '-1'"><div style="height: 27px;width: 27px;line-height: 27px;" class="lan">{{itemminall}}</div><span id="jia"> + </span></div>
                                </div>
                                <div  v-else-if="red.indexOf(itemminall) != '-1'" style="height: 27px;width: 27px;line-height: 27px;" class="hong">{{itemminall}}</div>
                                <div  v-else-if="green.indexOf(itemminall) != '-1'" style="height: 27px;width: 27px;line-height: 27px;" class="lv">{{itemminall}}</div>
                                <div  v-else="bulr.indexOf(itemminall) != '-1'" style="height: 27px;width: 27px;line-height: 27px;" class="lan">{{itemminall}}</div>
    ...
    export default{
        ...
        //   console.log(this.thisdatalist);
          ...
          mounted:function() {
            ...
            this.$nextTick(()=>{
                // document.getElementById("contented").style.height=document.getElementsByTagName("body")[0].offsetHeight-40 +"px"
            })
