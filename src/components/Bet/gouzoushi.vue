...
<div id="app" style="overflow: hidden;">
    <div id="box" style="top:0;width:100%;height:100%;background: white;z-index:1000;overflow: hidden;overflow-x: hidden;">
		<headertop :centerType="2" :rightType="2">
			<template slot="centerContent">
				<div class="color_species">
					<span style="vertical-align: middle; display: inline-block;">{{ cound }}</span>
					<span id="xx" style="display: inline-block;"><i class="iconfont"></i></span>
				</div>
		  ...
		  <main v-if="datacodea=='cqklsf'||datacodea=='gdklsf'" id="main" style="overflow: scroll; overflow-x: hidden;">
			 <div style="padding:  0;" class="number_Kj"> 
			 </div>
			 <div class="zoushiAll" style="font-size:16px;padding-top: 12px;line-height: .2rem;color:red;padding-top:12px;padding-bottom: 10px;">
				 <span style="display: inline-block;width: 1rem;height: .28rem;line-height: .28rem;background-color: #ff6b52;color: white;border-radius: .05rem;">
					 走势
				 </span>
			 </div>
			  <div class="ishppye">
		  ...
		  <main class="lhc" v-else-if="datacodea=='xglhc'||datacodea=='fflhc'||datacodea=='jslhc'" id="main" style="overflow: scroll; overflow-x: hidden;">
			  <div style="padding:  0;" class="number_Kj"> 
			 </div>
					...
					<div class="xglhcstyle">
						...
						<div class="wm_ulAllrigth">
							...
							<ul v-for="(numindex,index) in nummaxAll" :key='index'>
								<li>
									<span style="margin-left: .02rem; background: linear-gradient(-30deg, #fa3956, #ff6b52);" v-if="indexyi!=numindex.result.split(',').length-1&&red.indexOf(numindexmin) != '-1'" v-for="(numindexmin,indexyi) in numindex.result.split(',')" :key="indexyi">{{numindexmin}}</span>
									<span style="margin-left: .02rem;background:linear-gradient(90deg,  #34bdf2, #358af2);" v-else-if="indexyi!=numindex.result.split(',').length-1&&bulr.indexOf(numindexmin) != '-1'">{{numindexmin}}</span>
									<span style="margin-left: .02rem;background:linear-gradient(-30deg, rgb(43, 199, 146), rgb(134, 227, 109));" v-else-if="indexyi!=numindex.result.split(',').length-1&&green.indexOf(numindexmin) != '-1'">{{numindexmin}}</span>
								</li>
								<li>
									<span style="margin-left: .02rem; background: linear-gradient(-30deg, #fa3956, #ff6b52);" v-if="indexer==numindex.result.split(',').length-1&&red.indexOf(numindexmin) != '-1'" v-for="(numindexmin,indexer) in numindex.result.split(',')" :key='indexer'>{{numindexmin}}</span>
									<span style="margin-left: .02rem;background:linear-gradient(90deg, #34bdf2, #358af2);" v-else-if="indexer==numindex.result.split(',').length-1&&bulr.indexOf(numindexmin) != '-1'">{{numindexmin}}</span>
									<span style="margin-left: .02rem;background:linear-gradient(-30deg, rgb(43, 199, 146), rgb(134, 227, 109));" v-else-if="indexer==numindex.result.split(',').length-1&&green.indexOf(numindexmin) != '-1'">{{numindexmin}}</span>
								</li>
		  ...
		  <main class="hissq" v-else-if="datacodea=='ssq'||datacodea=='jsssq'" id="main" style="overflow: scroll; overflow-x: hidden;">
			 <div style="padding:  0;" class="number_Kj"> 
			 </div>
			 <div class="zoushiAll" style="font-size:16px;line-height: .2rem;color:red;padding: 10px 0;padding-top: 12px;">
				 <span style="display: inline-block;width: 1rem;height: .28rem;line-height: .28rem;background-color: #ff6b52;color: white;border-radius: .05rem;">
			  ...
			  <div class="ishppye">
				  <div class="xglhcstyle">
						...
						<div class="wm_ulAllrigth" style="overflow: hidden;">
							<ul style=" width: 100%;">
								<li style="width: 100%;">
									平码
								</li>
							...
							<ul v-for="(numindex,indexs) in nummaxAll" style="    width: 100%;" :key='indexs'>
								<li style="    width: 100%;color:#ff6b52">
									...
									<span style="margin-left: .06rem;background: linear-gradient(90deg, rgb(52, 189, 242), rgb(53, 138, 242));;" v-else>{{numindexmina}}</span>
								</li>
							</ul>
		  ...
		  <main v-else id="main" style="overflow: scroll; overflow-x: hidden;">
			<div class="number_Kj"> 
			...
			<div class="wm_con">
				<div class="wm_ulAll">
					...
					<ul><li></li></ul>
					<ul><li></li></ul>
					<div id='fg' style="height: .5rem;"></div>
					<div id="canvasdiv">
					</div>
				</div>
			</div>
	...
	computed: {
		...
		methods:{
			...
			getapi(){
				...
				console.log(111111+this.nummaxAll);
			...
			affairAll(){
				...
				$('.color_species').click(function(){
					topH = $(".header").height() + $(".color_speciesAll").height() +20 +"px"
					setTimeout(function(){
		...
		//			$('.color_species span:first-of-type').html('&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp')
					...
					if(fal){
						...
						$(".mengceng").css({"display":"block","top":topH})//蒙层
						fal=false
					}else{
						$('#main').css('overflow-y', 'scroll')
						$('#xx').css({"transform":"rotate(0deg)"});
						$(".mengceng").css("display","none")//蒙层
						fal=true;
					}
				});
				...
				mains.style.height=document.getElementsByTagName("body")[0].offsetHeight-30+'px'
		...
		trendchart(ids,w) {
			...
			cvs.style.top = - this.$store.state.headerHeight + 'px';
		...
		mounted:function(){
			...
			this.iszsAll();
			$('.index_header').css('z-index', '9999');
			setTimeout(function(){
				$this.isclickmr();
				var mains=document.getElementById("main");
				//mains.style.height=document.getElementsByTagName("body")[0].offsetHeight-75+'px'
