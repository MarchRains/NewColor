...
Vue.filter('dateFmttime', (input) => {
    var input = Math.round(parseFloat(input) * 100) / 100;
			var xsd = input.toString().split(".");
			if(input==4.93){
                input =4.94;
                return input;
            }
			if(xsd.length == 1) {
				input = input.toString() + ".00";
				return input;
			};
			if(xsd.length > 1) {
				if(xsd[1].length < 2) {
...
Vue.filter('getDictValue', (value, tableName,fieldName) => {
    var dict = JSON.parse(myStorage.getItem("dictDetailList"));
    var key = tableName+'-' + fieldName + '-' +value;
    return dict[key];
})
...
Vue.prototype.msg= function (msg) {
    ...
    for(var a = 0;a<msg.length;a++){
        str+='<span style="display: inline-block;margin-top: .02rem;width: .21rem;height: .23rem;margin-right: .03rem;text-shadow: 0 1px #000, 1px 0 #000, -1px 0 #000, 0 -1px #000;background: linear-gradient(90deg, '+colorAll[msg[a]]+', '+colorAll[msg[a]]+');color: white;line-height: .23rem;text-align: center;border-radius: 15%;font-size: .17rem;">'+Number(msg[a])+'</span>'
    }
    return str
...
// #ff6b52
// import './assets/source/css/common/color.css'
// var h5Style = JSON.parse(localStorage.getItem("sysConfigMap")).h5Style.cvalue;
// window.ifred = false
// if (h5Style == 1) {
//     console.log('red')
//     import("./assets/source/css/common/color-red.css")
...
// else if (h5Style == 2) {
//     console.log('yellow')
//     import("./assets/source/css/common/color-yellow.css")
//     window.mapcolor = "#fe8e42"
//     window.bgmapcolor = "linear-gradient(90deg,  #fe8e42, #ffba02)"
//     window.ifyellow = true
// } else if (h5Style == 3) {
//     console.log('blue')
//     import("./assets/source/css/common/color-blue.css")
//     window.mapcolor = "#3b89ed"
//     window.bgmapcolor = "linear-gradient(90deg, #3b89ed, #05bcfe)"
//     window.ifblue = true
// } else if (h5Style == 4) {
//     console.log('green')
//     import("./assets/source/css/common/color-green.css")
//     window.mapcolor = "#40d95a"
//     window.bgmapcolor = "linear-gradient(90deg, #12b84a, #40d95a)"
//     window.ifgreen = true
// }
// else if (window.h5Style == 5) {
//     // require("./assets/source/css/common/color-purple.css")
//     import("./assets/source/css/common/color-red.css")
//     window.mapcolor = "#ba32fe"
//     window.bgmapcolor = "linear-gradient(90deg, #ba32fe, #671aff)"
//     window.ifpurple = true
// }
...
window.vm=new Vue({
    ...
    mounted:function(){
        // jumpCustomerService("https://f18.livechatvalue.com/chat/chatClient/chatbox.jsp?companyID=876510&configID=72957&jid=8025085340&s=1")
    ...
    methods: {
        ...
        ishaph(){
			// 如果当前页面非注册页，显示首屏页
			if (!/^\/(register)/.test(this.$route.path)) this.showScreen()
		...
		showScreen() {
			var
			el		= $('<div class="screen"><div><span></span></div></div>'),
			target	= el.find('span');
			target.hide();
			$('body').append(el);
			setTimeout(() => {
		...
		// 关闭首屏页
		hideScreen() {
			var el	= $('body').children('.screen');
			el.fadeOut(400);
		},
        getrulcanshu() {
            // 是否是内嵌
            let isApp = utils.hasUrlParams('isApp');
            // （前提为内嵌）刘海的高度
            let lh = utils.getUrlParams()['lh'];
            //var lociosAndroid=localStorage.getItem("iosAndroid")
            this.$store.state.isApp = +isApp;
            var h = 50;
            // statusbar 的高度（包括刘海）
            this.$store.state.statusbarHeight = isApp ? (+lh ? +lh : 20) : 0;
            if (isApp) h += +lh ? +lh : 20;
            // 普通页面头部高度
            this.$store.state.headerHeight = h;
            console.info('是否是内嵌：', this.$store.state.isApp);
            console.info('页面头部高度：', h);
        },
         getPath() {
             // 回到代理页面，清空注单和帐变记录的筛选数据 初始化日期为今天
            if(this.$route.path == "/daili"){
                this.$store.state.ZhudanstartDate=new Date().YMD(0);
                this.$store.state.ZhudanendDate=new Date().YMD(0);
                this.$store.state.ZhudanuserName="";
                this.$store.state.ZhudanLotteryNum="";
                this.$store.state.ZhudanLotteryId="";
                this.$store.state.billstartDate=new Date().YMD(0);
                this.$store.state.billendDate=new Date().YMD(0);
                this.$store.state.billuserName="";
                this.$store.state.billColor="";
                this.$store.state.teamstartDate="";
                this.$store.state.teamendDate="";
                this.$store.state.teamUserName="";
                this.$store.state.teamStatus="";
                this.$store.state.Zhusername="";
                this.$store.state.bChangeName="";
            }
            //回到团队页面清空之前的搜索用户名
            if(this.$route.path == '/agencyDown'){
                this.$store.state.Zhusername="";
                this.$store.state.ZhudanuserName=""
                this.$store.state.bChangeName="";
                this.$store.state.billuserName=""                
