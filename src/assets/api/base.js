 window.base = {
	...
	domain: "http://lottery-test-bucket2.oss-cn-hongkong.aliyuncs.com/",
	 hostUrl :"http://47.75.87.241/",//http://47.91.168.10/
	socketType:"ws",
	zipUrl:'common/loadBaseDataZip',
	local: ":///47.75.87.241/common/socket?timestamp=",
...
// 5000ce
	//  domain: "http://47.91.174.36/",
	//  hostUrl: "http://47.91.174.36/",//http://47.91.168.10/
	//      socketType: "ws",
	//      zipUrl: 'common/loadBaseDataZip',
	//      local: "://47.91.174.36/common/socket?timestamp=",
		...
		function dealNew(url, data) {
			...
			return $.ajax({
				...
				success: function (data1) {
					...
					if(myStorage.getItem("time-version")!= null && data1.version && data1.version > myStorage.getItem("time-version").split("-")[1] && !isLoadingDataBase) {
						$.tips("版本已更新，正在为您刷新页面");
						setTimeout(()=>{
...
//---------------------------------------------------以下是接口调用
			...
			getIp: function () {
				...
				//	var result = deal('common/getTimestamp', data);
				var result = dealNew('common/fresh/getIp', data);
				return result
			},
		   ...
		   // 获取二维码地址
			...
			//获取当日中奖
			userCurrDayPro: function(userId){
				var data = {
					timestamp: new Date().getTime() - myStorage.getItem("time-version").split("-")[0],
					token: myStorage.getItem("token"),
					userId:userId,
				};
				return dealNew('user/userCurrDayPro',data)
			 },
			 //彩种介绍
 ...
 */
			 ...
			 * @returns
			 */
			ploxyApply:function (account, password, device, mobile,email,qq,weChat,reason) {
				var data = {
					timestamp: new Date().getTime() - myStorage.getItem("time-version").split("-")[0],
					account: account,
					password: password,
			 ...
			 * 获取所有用户下级用户注单
			 */
			subUserBettingList:function (userId, ploxyId,startTime,endTime, account,lotteryId,lotteryNo,pageNumber,pageSize) {
				var data = {
					token: myStorage.getItem("token"),
					timestamp: new Date().getTime() - myStorage.getItem("time-version").split("-")[0],
					userId: userId,
			...
			//	var result = deal('user/ploxy/modifyProxy', data);
				var result = deal('user/ploxy/subUserBettingList', data);
			 ...
			 *  获取下级用户金流
			 */
			subUserAccountDetails:function (userId, ploxyId,startTime,endTime, account,detailType,pageNumber,pageSize) {
				var data = {
					token: myStorage.getItem("token"),
					timestamp: new Date().getTime() - myStorage.getItem("time-version").split("-")[0],
					userId: userId,
			...
			//	var result = deal('user/ploxy/modifyProxy', data);
				var result = deal('user/ploxy/subUserAccountDetails', data);
				if(!result || result.code != 200){
					$.tips(result.message, 1000);
				}
				return result;
			...
			ploxyList1: function (pageNumber, pageSize, userId, uid, account, beginTime, endTime,onlineStatus) {
				var data = {
					token: myStorage.getItem("token"),
					timestamp: new Date().getTime() - myStorage.getItem("time-version").split("-")[0],
					userId: userId,
